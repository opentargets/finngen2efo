...
endpoints <- read_csv("../data/FINNGEN_ENDPOINTS_DF2_FINAL_2018-11-29_Public.csv",
                      guess_max = 3000,
                      comment = "")
#manifest <- read_tsv("../temp/finngen_manifest.tsv")
phenotypes <- read_tsv("../temp/finngen_phenotypes.tsv",
...
zooma <- read_tsv("../temp/ontoma_results.tsv")
...
result <- endpoints %>%
    ...
    bind_rows(zooma %>%
              ...
              select(LONGNAME = query,
                     ...
                     quality) %>%
              left_join(manifest %>%
                        select(NAME = phenocode, LONGNAME = name),
                        by = "LONGNAME")) %>%
...
result %>%
    write_csv("~/Desktop/test_mappings.csv")
